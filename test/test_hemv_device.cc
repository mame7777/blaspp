...
static HemvDeviceMemoryPool<float, float, float> pool_s;
static HemvDeviceMemoryPool<double, double, double> pool_d;
static HemvDeviceMemoryPool<std::complex<float>, std::complex<float>, std::complex<float>> pool_c;
static HemvDeviceMemoryPool<std::complex<double>, std::complex<double>, std::complex<double>> pool_z;
...
void test_hemv_device_work( Params& params, bool run )
{
    ...
    if (std::is_same<TA, float>::value && std::is_same<TX, float>::value && std::is_same<TY, float>::value) {
        ...
        pool = reinterpret_cast<HemvDeviceMemoryPool<TA, TX, TY>*>(&pool_c);
        ...
        pool = reinterpret_cast<HemvDeviceMemoryPool<TA, TX, TY>*>(&pool_z);
    }
