...
static TrsvDeviceMemoryPool<float, float> pool_s;
static TrsvDeviceMemoryPool<double, double> pool_d;
static TrsvDeviceMemoryPool<std::complex<float>, std::complex<float>> pool_c;
static TrsvDeviceMemoryPool<std::complex<double>, std::complex<double>> pool_z;
...
void test_trsv_device_work( Params& params, bool run )
{
    ...
    TrsvDeviceMemoryPool<TA, TX>* pool = nullptr;
    if (std::is_same<TA, float>::value && std::is_same<TX, float>::value) {
        pool = reinterpret_cast<TrsvDeviceMemoryPool<TA, TX>*>(&pool_s);
    } else if (std::is_same<TA, double>::value && std::is_same<TX, double>::value) {
        pool = reinterpret_cast<TrsvDeviceMemoryPool<TA, TX>*>(&pool_d);
    } else if (std::is_same<TA, std::complex<float>>::value && std::is_same<TX, std::complex<float>>::value) {
        pool = reinterpret_cast<TrsvDeviceMemoryPool<TA, TX>*>(&pool_c);
    } else if (std::is_same<TA, std::complex<double>>::value && std::is_same<TX, std::complex<double>>::value) {
        pool = reinterpret_cast<TrsvDeviceMemoryPool<TA, TX>*>(&pool_z);
    }
    ...
    pool->allocate(size_A, size_x, device);
