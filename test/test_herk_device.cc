...
static HerkDeviceMemoryPool<float, float> pool_s;
static HerkDeviceMemoryPool<double, double> pool_d;
static HerkDeviceMemoryPool<std::complex<float>, std::complex<float>> pool_c;
static HerkDeviceMemoryPool<std::complex<double>, std::complex<double>> pool_z;
...
void test_herk_device_work( Params& params, bool run )
{
    ...
    HerkDeviceMemoryPool<TA, TC>* pool;
    if (std::is_same<TA, float>::value) {
        pool = reinterpret_cast<HerkDeviceMemoryPool<TA, TC>*>(&pool_s);
    }
    else if (std::is_same<TA, double>::value) {
        pool = reinterpret_cast<HerkDeviceMemoryPool<TA, TC>*>(&pool_d);
    }
    else if (std::is_same<TA, std::complex<float>>::value) {
        pool = reinterpret_cast<HerkDeviceMemoryPool<TA, TC>*>(&pool_c);
    }
    else {
        pool = reinterpret_cast<HerkDeviceMemoryPool<TA, TC>*>(&pool_z);
    }
