...
static Nrm2DeviceMemoryPool<float> pool_s;
static Nrm2DeviceMemoryPool<double> pool_d;
static Nrm2DeviceMemoryPool<std::complex<float>> pool_c;
static Nrm2DeviceMemoryPool<std::complex<double>> pool_z;
...
void test_nrm2_device_work( Params& params, bool run )
{
    ...
    Nrm2DeviceMemoryPool<T>* pool = nullptr;
    if (std::is_same<T, float>::value) {
        pool = reinterpret_cast<Nrm2DeviceMemoryPool<T>*>(&pool_s);
    } else if (std::is_same<T, double>::value) {
        pool = reinterpret_cast<Nrm2DeviceMemoryPool<T>*>(&pool_d);
    } else if (std::is_same<T, std::complex<float>>::value) {
        pool = reinterpret_cast<Nrm2DeviceMemoryPool<T>*>(&pool_c);
    } else if (std::is_same<T, std::complex<double>>::value) {
        pool = reinterpret_cast<Nrm2DeviceMemoryPool<T>*>(&pool_z);
    }
