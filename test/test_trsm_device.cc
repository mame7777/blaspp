...
static TrsmDeviceMemoryPool<float, float> pool_s;
static TrsmDeviceMemoryPool<double, double> pool_d;
static TrsmDeviceMemoryPool<std::complex<float>, std::complex<float>> pool_c;
static TrsmDeviceMemoryPool<std::complex<double>, std::complex<double>> pool_z;
...
void test_trsm_device_work( Params& params, bool run )
{
    ...
    TrsmDeviceMemoryPool<TA, TB>* pool;
    if (std::is_same<TA, float>::value) {
        pool = reinterpret_cast<TrsmDeviceMemoryPool<TA, TB>*>(&pool_s);
    }
    else if (std::is_same<TA, double>::value) {
        pool = reinterpret_cast<TrsmDeviceMemoryPool<TA, TB>*>(&pool_d);
    }
    else if (std::is_same<TA, std::complex<float>>::value) {
        pool = reinterpret_cast<TrsmDeviceMemoryPool<TA, TB>*>(&pool_c);
    }
    else {
        pool = reinterpret_cast<TrsmDeviceMemoryPool<TA, TB>*>(&pool_z);
    }
