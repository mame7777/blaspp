...
static GeaddDeviceMemoryPool<float> pool_s;
static GeaddDeviceMemoryPool<double> pool_d;
static GeaddDeviceMemoryPool<std::complex<float>> pool_c;
static GeaddDeviceMemoryPool<std::complex<double>> pool_z;
...
void test_geadd_device_work( Params& params, bool run )
{
    ...
    GeaddDeviceMemoryPool<scalar_t>* pool;
    if (std::is_same<scalar_t, float>::value) {
        pool = reinterpret_cast<GeaddDeviceMemoryPool<scalar_t>*>(&pool_s);
    }
    else if (std::is_same<scalar_t, double>::value) {
        pool = reinterpret_cast<GeaddDeviceMemoryPool<scalar_t>*>(&pool_d);
    }
    else if (std::is_same<scalar_t, std::complex<float>>::value) {
        pool = reinterpret_cast<GeaddDeviceMemoryPool<scalar_t>*>(&pool_c);
    }
    else {
        pool = reinterpret_cast<GeaddDeviceMemoryPool<scalar_t>*>(&pool_z);
    }
